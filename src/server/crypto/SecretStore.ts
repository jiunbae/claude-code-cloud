...
function getEncryptionKey(): Buffer {
  ...
  if (!keyBase64) {
    ...
    if (jwtSecret) {
      ...
      // Use environment-specific salt to prevent rainbow table attacks across different installations
      const salt = process.env.DEV_CRYPTO_SALT || crypto.randomBytes(16).toString('hex');
