...
// POST /api/sessions/:id/shell/start - Start shell terminal process
export async function POST(_request: NextRequest, { params }: RouteParams) {
  ...
  try {
    const response = await fetch(`${PTY_API_URL}/sessions/${id}/shell/start`, {
      method: 'POST',
    ...
    return NextResponse.json(
      { error: `Failed to start shell: ${(error as Error).message}` },
      { status: 500 }
    );
  }
}
