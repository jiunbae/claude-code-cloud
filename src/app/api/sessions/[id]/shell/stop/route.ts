...
// POST /api/sessions/:id/shell/stop - Stop shell terminal process
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    const response = await fetch(`${PTY_API_URL}/sessions/${id}/shell/stop`, {
      method: 'POST',
    ...
    if (!response.ok) {
      // If shell not running, treat as already stopped
      if (response.status === 400) {
        return NextResponse.json({ success: true, message: 'Shell already stopped' });
      }
    ...
    return NextResponse.json(
      { error: `Failed to stop shell: ${(error as Error).message}` },
      { status: 500 }
    );
  }
}
